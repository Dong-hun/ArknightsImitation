...
public class EnemyManager : MonoBehaviour
{
    ...
    protected enum STATE                   // 적 상태
    {                                   
        IDLE, MOVE, BATTLE, DIE         // 대기, 움직임, 전투, 사망(필요시 추가삭제 하시면 됩니당)
    }
    protected STATE m_State;               // 상태값 저장 변수
    NavMeshAgent m_Navi;                // NavMeshAgent 컴포넌트 접근용 변수

    protected Vector3 m_StartPos;          // 출발지 (임시)
    Vector3 _dest;                      // 목적지
    ...
    void Start()
    {
        ...
        m_Dest = Vector3.zero;          // 도착지 설정
        m_Navi.SetDestination(m_Dest);  // 네비게이션에서도 도착지 설정
        m_State = STATE.MOVE;           // 상태를 움직임으로 설정해서 바로 이동하게 함
    }
