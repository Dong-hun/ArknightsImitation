...
public class NodeManager : MonoBehaviour
{
    ...
    public int NTileX;                              //Node tileX값저장
    public int NTileY;                              //Node tileY값저장
    public int TileX;                              //Tile tileX값저장
    public int TileY;                              //Tile tileY값저장
    ...
    public void Build()
    {
        ...
        if (Physics.Raycast(ray, out hit, Mathf.Infinity))
        {
            ...
            if (hit.transform.gameObject.layer == LayerMask.NameToLayer("Ground"))
            {
                ...
                Debug.LogError(m_TileState[tileY, tileX].ToString() + tileY + "" + tileX);
                ...
                if (m_TileState[NTileY, NTileX] == TILEINFO.NONE)
                {
                    //업그레이드, 삭제 UI비활성화
                ...
                //이건 혹시몰라서 만들어 놨어요
                else if (m_TileState[NTileY, NTileX] == TILEINFO.TOWER)
                {
            ...
            else if (hit.transform.gameObject.layer == LayerMask.NameToLayer("Tower"))
            {
                ...
                //해당 타워의 타일번호를 console로 보기위해 임의로 넣어놓음(지워도 상관없음)
                Debug.LogError(m_TileState[tileY, tileX].ToString() + tileY + "" + tileX);

                //타워 tileY값과 tileX값을 TileY,TileX 에 넣어줌 (나중에 BuildManager 스크립트에서 m_TileState를 변경하기 위해사용)
                TileX = tileX;
                TileY = tileY;

                //처음 state값을 Tower로 초기화해준다.
                ...
                if (m_TileState[TileY, TileX] == TILEINFO.TOWER)
                {
                    ...
                    //충돌된 Tower를 m_Tower에 담음 (BuildManager에서 충돌된 타워만 삭제하기위함)
                    m_Tower = hit.transform.gameObject;
